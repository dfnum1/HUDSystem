	...
	SubShader{
		...
		Pass {
			...
			pixel_t VertShader(vertex_t input)
			{
				...
				float2 spriteUVPos = fontAtlasMapping(spriteid * 2) - float2(padding, padding);
				float2 spriteUVSize = fontAtlasMapping(spriteid * 2 + 1) + 2 * float2(padding, padding);
				input.vertex.xy = spritePos + input.vertex.xy * spriteSize;
				rotate2D(input.vertex.xy, angle);
				input.vertex.xy = input.vertex.xy + comPos;

				input.texcoord0 = (spriteUVPos + input.texcoord0 * spriteUVSize) / float2(_TextureWidth, _TextureHeight);
				...
				output.texcoord0 = float4(input.texcoord0.x, input.texcoord0.y, boldparam, boldparam);
