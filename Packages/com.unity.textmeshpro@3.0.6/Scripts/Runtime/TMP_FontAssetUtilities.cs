...
namespace TMPro
{
    public class TMP_FontAssetUtilities
    {
        ...
        static List<FontPathInfo> systemFontPath;

        private static List<FontPathInfo> GetSysFontPath()
        {
            if (systemFontPath != null)
            {
                return systemFontPath;
            }
            systemFontPath = new List<FontPathInfo>();
            string[] tempPaths = UnityEngine.Font.GetPathsToOSFonts();
            FaceInfo deffaceInfo = TMP_Settings.defaultFontAsset.faceInfo;
            for (int i = 0; i < tempPaths.Length; i++)
            {
                if (FontEngine.LoadFontFace(tempPaths[i], deffaceInfo.pointSize) == FontEngineError.Success)
                {
                    FaceInfo faceInfo = FontEngine.GetFaceInfo();
                    if (faceInfo.styleName == deffaceInfo.styleName)
                    {
                        FontPathInfo fontPath = new FontPathInfo();
                        fontPath.path = tempPaths[i];
                        fontPath.faceInfo = faceInfo;
                        fontPath.sortIndex = FontSort(fontPath.path);
                        systemFontPath.Add(fontPath);
                    }
                }
            }
            systemFontPath.Sort((a, b) =>
            {
        ...
        private static int FontSort(string familyName)
        {
#if UNITY_EDITOR
            ...
            if (familyName.Contains("Segoe"))
            {
                return 2;
            }
            if (familyName.Contains("Roman"))
            {
                return 3;
            }
            return 1000;
#elif UNITY_ANDROID
            if (familyName.Contains("Roboto"))
            {
                return 1;
            }
            if (familyName.Contains("NotoSansThaiUI"))
            {
                return 2;
            }
            if (familyName.Contains("Noto Serif"))
            {
                return 3;
            }
            if (familyName.Contains("Droid Sans"))
            {
                return 4;
            }
            return 1000;
#elif UNITY_IOS
            if (familyName.Contains("Helvetica"))
            {
                return 1;
            }
            if (familyName.Contains("Helvetica Neue"))
            {
                return 2;
            }
            if (familyName.Contains("Arial"))
        ...
        public static TMP_Character GetCharacterFormSysFont(uint unicode, FontStyles fontStyle, FontWeight fontWeight, out bool isAlternativeTypeface)
        {
            isAlternativeTypeface = false;
            TMP_Character character = null;
            List<TMP_FontAsset> fallbackFontAssetTable = TMP_Settings.defaultFontAsset.fallbackFontAssetTable;
            for (int i = fallbackFontAssetTable.Count - 1; i >= 0; i--)
            {
                if (fallbackFontAssetTable[i] == null)
                {
                    fallbackFontAssetTable.RemoveAt(i);
                }
            }
            List<FontPathInfo> systemFontPath = GetSysFontPath();
            for (int i = 0; i < systemFontPath.Count; i++)
            {
                if (FontEngine.LoadFontFace(systemFontPath[i].path) == FontEngineError.Success)
                {
                    uint index = FontEngine.GetGlyphIndex(unicode);
                    if (index != 0)
                    {
                        TMP_ExpandFontAssets fontAsset = TMP_ExpandFontAssets.CreateExpandFontAssets(systemFontPath[i].path, TMP_Settings.defaultFontAsset);
                        fontAsset.name = systemFontPath[i].path;
                        character = GetCharacterFromFontAsset_Internal(unicode, fontAsset, false, fontStyle, fontWeight, out isAlternativeTypeface);
                        TMP_Settings.defaultFontAsset.fallbackFontAssetTable.Add(fontAsset);
                        return character;
                    }
                }
            }
