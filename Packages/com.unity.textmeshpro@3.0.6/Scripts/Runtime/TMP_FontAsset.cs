...
namespace TMPro
{
    ...
    public class TMP_FontAsset : TMP_Asset
    {
        ...
        {
            get { return m_SourceFontPath; }
            internal set { m_SourceFontPath = value; }
        }
        [SerializeField]
        private string m_SourceFontPath;

        private FontEngineError LoadFontFace(int pointSize)
        {
            if (!string.IsNullOrEmpty(m_SourceFontPath))
            {
                return FontEngine.LoadFontFace(m_SourceFontPath, pointSize);
            }
            return FontEngine.LoadFontFace(m_SourceFontFile, pointSize);
        }
        ...
        /// </summary>
        public virtual List<TMP_Character> characterTable
        {
        ...
        public int fontMappingWidth 
        {
            get 
            {
                if (fontMappingTexture == null) return 0;
                return fontMappingTexture.width;
            }
        }

        public int fontMappingHeight
        {
            get
            {
                if (fontMappingTexture == null) return 0;
                return fontMappingTexture.height;
            }
        }
        ...
        internal List<TMP_Character> m_fontCharacter = new List<TMP_Character>();
        public virtual List<TMP_Character> fontCharacter
        {
            get { return m_fontCharacter; }
        }

        public virtual void AddCharacter(TMP_Character character)
        {
            character.index = fontCharacter.Count;
            characterTable.Add(character);
            fontCharacter.Add(character);
            AddAtlasMapping(character);
        }

        public void SetFontAtlasMapping(Texture2D fontAtlasMappingTextures)
        {
            m_FontMappingTexture = fontAtlasMappingTextures;
            ClearFontAtlasMapping();
        }

        private void ClearFontAtlasMapping()
        {
            if (fontMappingTexture == null) return;
            FontEngine.ResetAtlasTexture(fontMappingTexture);
            fontMappingTexture.Apply();
        }

        private void ResetAtlasMapping()
        {
            for (int i = 0; i < characterTable.Count; i++)
            {
                ...
                WriteUV(glyphRect.x, glyphRect.y, character.index);
                WriteUV(glyphRect.width, glyphRect.height, character.index);
            }
            fontMappingTexture.Apply();
        }

        private void AddAtlasMapping(TMP_Character character)
        {
            if (fontMappingTexture == null) return;
            int maxsize = fontMappingWidth * fontMappingHeight;
            int presize = (character.index + 1) * 2;
            if (presize > maxsize)
            {
            ...
            GlyphRect glyphRect = character.glyph.glyphRect;
            WriteUV(glyphRect.x, glyphRect.y, character.index * 2);
            WriteUV(glyphRect.width, glyphRect.height, character.index*2 + 1);
            fontMappingTexture.Apply();
        }

        private void WriteUV(int x, int y, int index)
        {
            byte r = (byte)(x / 256);
            byte g = (byte)(x % 256);
            byte b = (byte)(y / 256);
            ...
            int h = index / fontMappingWidth;
            int w = index % fontMappingWidth;
            fontMappingTexture.SetPixel(w, h, color);
        }
        ...
        /// </summary>
        public virtual Texture2D[] atlasTextures
        {
            get
            {
        ...
        internal virtual int atlasTextureIndex 
        {
            get { return m_AtlasTextureIndex; }
            set { m_AtlasTextureIndex = value; }
        }

        /// <summary>
        /// Number of atlas textures used by this font asset.
        /// </summary>
        public int atlasTextureCount { get { return atlasTextureIndex + 1; } }
        ...
        /// </summary>
        internal virtual List<GlyphRect> usedGlyphRects
        {
        ...
        /// </summary>
        internal virtual List<GlyphRect> freeGlyphRects
        {
        ...
        internal void AddSynthesizedCharactersAndFaceMetrics()
        {
            ...
            if (m_AtlasPopulationMode == AtlasPopulationMode.Dynamic)
                isFontFaceLoaded = LoadFontFace(m_FaceInfo.pointSize) == FontEngineError.Success;
        ...
        internal void SortCharacterTable()
        {
            if (characterTable != null && characterTable.Count > 0)
                characterTable = characterTable.OrderBy(c => c.unicode).ToList();
        }
        ...
        internal uint GetGlyphIndex(uint unicode)
        {
            ...
            // Load font face.
            if (LoadFontFace(m_FaceInfo.pointSize) != FontEngineError.Success)
                return 0;
        ...
        public bool TryAddCharacters(uint[] unicodes, out uint[] missingUnicodes, bool includeFontFeatures = false)
        {
            ...
            if (atlasTextures[atlasTextureIndex].width == 0 || atlasTextures[atlasTextureIndex].height == 0)
            {
                atlasTextures[atlasTextureIndex].Reinitialize(m_AtlasWidth, m_AtlasHeight);
                FontEngine.ResetAtlasTexture(atlasTextures[atlasTextureIndex]);
            }

            Glyph[] glyphs;
            bool allGlyphsAddedToTexture = FontEngine.TryAddGlyphsToTexture(m_GlyphsToAdd, m_AtlasPadding, GlyphPackingMode.BestShortSideFit, freeGlyphRects, usedGlyphRects, m_AtlasRenderMode, atlasTextures[atlasTextureIndex], out glyphs);
        ...
        */
        ...
        internal bool TryAddCharacterInternal(uint unicode, out TMP_Character character)
        {
            ...
            if (FontEngine.TryAddGlyphToTexture(glyphIndex, m_AtlasPadding, GlyphPackingMode.BestShortSideFit, freeGlyphRects, usedGlyphRects, m_AtlasRenderMode, atlasTextures[atlasTextureIndex], out glyph))
            {
        ...
        void SetupNewAtlasTexture()
        {
            ...
            if (atlasTextures.Length == atlasTextureIndex)
            {
                Texture2D[] m_AtlasTextures = atlasTextures;
                Array.Resize(ref m_AtlasTextures, atlasTextures.Length * 2);
                atlasTextures = m_AtlasTextures;
            }

            // Initialize new atlas texture
            atlasTextures[atlasTextureIndex] = new Texture2D(m_AtlasWidth, m_AtlasHeight, TextureFormat.Alpha8, false);
            FontEngine.ResetAtlasTexture(atlasTextures[atlasTextureIndex]);
            ...
            freeGlyphRects.Add(new GlyphRect(0, 0, m_AtlasWidth - packingModifier, m_AtlasHeight - packingModifier));
            ...
            if (UnityEditor.EditorUtility.IsPersistent(this))
            {
                Texture2D tex = atlasTextures[atlasTextureIndex];
                tex.name = m_AtlasTexture.name + " " + atlasTextureIndex;

                UnityEditor.AssetDatabase.AddObjectToAsset(atlasTextures[atlasTextureIndex], this);
        ...
        internal void UpdateFontAssetData()
        {
            ...
            uint[] unicodeCharacters = new uint[characterTable.Count];

            for (int i = 0; i < characterTable.Count; i++)
                unicodeCharacters[i] = characterTable[i].unicode;
        ...
        internal void ClearAtlasTextures(bool setAtlasSizeToZero = false)
        {
            ...
            Texture2D[] mAtlasTextures = atlasTextures;
            Array.Resize(ref mAtlasTextures, 1);
            atlasTextures = mAtlasTextures;

            texture = m_AtlasTexture = atlasTextures[0];
        ...
        internal void UpgradeFontAsset()
        {
            ...
            if (!isSpaceCharacterPresent)
            {
                ...
                m_GlyphTable.Add(glyph);
                AddCharacter(new TMP_Character(32, this, glyph));
            }
